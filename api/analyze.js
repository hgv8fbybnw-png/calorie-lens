...
// =============================================================
// カロリーレンズ 解析API（モード対応版）
//   mode = "package": 画像。バーコード優先 → 商品名検索 → 印字。
//   mode = "eatout" : 画像。料理を見た目と量から推定（estimate）。
//   mode = "home"   : 食材リスト(name + grams)。日本食品標準成分表ベースで計算（recipe）。
...
const HOME_PROMPT = [
  'あなたは日本食品標準成分表(八訂)に精通した管理栄養士です。以下の食材と分量(グラム)から、各食材の栄養(カロリーとPFC)を成分表に基づき計算してください。Google検索ツールを使えます。',
  '食材リスト(JSON): {LIST}',
  '【厳守】各食材について、日本食品標準成分表の「100gあたり」の値を確認し、入力グラム数で比例計算する。生・ゆで等で大きく変わる場合は一般的な調理状態を想定する。確認できない食材は最も近い一般的な食材で代用し、その旨を name に補足する。',
  '【出力】説明やマークダウンを付けず、次のJSONのみ: { "items": [ { "name": "食材名", "amount": "分量g", "source": "recipe", "kcal": 数値, "protein_g": 数値, "fat_g": 数値, "carbs_g": 数値 } ] } itemは入力食材ごとに1つ。数値は数字のみ小数1桁。'
].join('\n');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (mode === 'home') {
      ...
      const list = ingredients.map(function (x) { return { name: String(x.name || ''), grams: num(x.grams) }; });
